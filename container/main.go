...
func init() {
	if _, exists := initializers["contInitAndRun"]; exists {
		panic(fmt.Sprintf("initializer already registered under name %q", "contInitAndRun"))
	}

	initializers["contInitAndRun"] = contInitAndRun
